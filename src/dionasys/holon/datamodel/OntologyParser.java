...
public class OntologyParser {
	...
	private static ConcurrentHashMap<String, String> instancesClassesMap = new ConcurrentHashMap<>(); // key = instance,
																										// value = class
	...
	public static HolonDataModel parse(InputStream inputStream) throws OWLOntologyCreationException {
		...
		// holonOntology.getOWLOntologyManager().getOntologyDocumentIRI(holonOntology));
	...
	public static HolonDataModel parse(OWLOntology holonOntology) throws OWLOntologyCreationException {
		...
		OWLClass holonParameters = dataFactory.getOWLClass(IRI.create(ontologyIRI + "#HolonParameters"));
		holon.setAllData(getIndividualsValues(holonParameters, holonOntology));
...
//                    }
	...
	/**
	 * returns the HolonDataModel Parameters
	 *
	 * @param individual
	 * @return
	 */
	private static ArrayList<OWLNamedIndividual> getHolonParameters(OWLNamedIndividual individual,
			OWLOntology holonOntology) {
		...
		while (iterObject.hasNext()) {
			...
			if (axiomObject
					.containsEntityInSignature(dataFactory.getOWLObjectProperty(ontologyIRI + "#hasParameter"))) {
	...
	/**
	 * returns the HolonDataModel services
	 *
	 * @param holonServices
	 * @param holonOntology
	...
	private static ArrayList<HolonServiceModel> getHolonServices(OWLClass holonServices, OWLOntology holonOntology) {
		...
		while (iterIndividual.hasNext()) {
					...
					while (compIter.hasNext()) {
						...
						if (!comp.getIRI().getIRIString().equals(individual.getIRI().getIRIString())) {
							...
							while (iter2.hasNext()) {
								...
								parameters.add(new ParameterModel(parameterName, TypeMapper.mapFromOWLTypes(literal)));
							}
						}
					} // while (compIter.hasNext()
					...
					while (compIter.hasNext()) {
						...
						if (!comp.getIRI().getIRIString().equals(individual.getIRI().getIRIString())) {
							...
							if (iter2.hasNext()) {
								OWLDataPropertyAssertionAxiom axiom = iter2.next();
								OWLLiteral literal = axiom.getObject();
								returnDataType = TypeMapper.mapFromOWLTypes(literal);
							}
						}
					}
				}
			...
			// Data --> attributes: name, cost, return
			Stream<OWLDataPropertyAssertionAxiom> axData = holonOntology.dataPropertyAssertionAxioms(individual);
			...
			while (iterData.hasNext()) {
				...
				if (key.equals("name")) {
					...
					ser.setAnnotation(Integer.parseInt(value));
				}
			}
	...
	/**
	 * get values of all individual of a class
	 * 
	 * @param clazz
	 */
	private static HashMap<String, String> getIndividualsValues(OWLClass clazz, OWLOntology holonOntology) {
		HashMap<String, String> data = new HashMap<>();
		NodeSet<OWLNamedIndividual> instances = reasoner.getInstances(clazz, false);
		Iterator<OWLNamedIndividual> iterIndividual = instances.entities().iterator();
		while (iterIndividual.hasNext()) {
			OWLNamedIndividual ind = iterIndividual.next();
			data.put(ind.getIRI().getShortForm(), getDataPropertyValues(ind, holonOntology));
		}
		return data;
	}

	/**
	 * get named individual of a class
	 * 
	 * @param clazz
	 */
	private static OWLNamedIndividual getNamedIndividual(OWLClass clazz, String indName) {
		NodeSet<OWLNamedIndividual> instances = reasoner.getInstances(clazz, false);
		Iterator<OWLNamedIndividual> iterIndividual = instances.entities().iterator();
		while (iterIndividual.hasNext()) {
			OWLNamedIndividual ind = iterIndividual.next();
			if (ind.getIRI().getShortForm().equals(indName))
				return ind;
		}
		return null;
	}
	 ...
	 * classes in a map key = instances (in a short form), value = class (in a short
	 * form)
	 */
