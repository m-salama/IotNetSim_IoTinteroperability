...
public class UserSmartPhone extends IoTNode  implements IoTNodeMobile {
	...
	public UserSmartPhone(String name, 
		...
		//to find the first nearest node and incremented to find the next available node
	...
	private void processVehicleFuelAlert() {
		...
		Log.printLine(CloudSim.clock() + ": [" + this.getName() + "] is processing the fuel alert received of the vehicle " 
		...
		schedule(this.getForwardNodeId(), CloudSim.getMinTimeBetweenEvents(), CloudSimTags.IOV_SEARCH_NEAREST_STATION_EVENT, 
				newData);
	...
	/**
	 * processSearchNearestStation()
	 * to handle a request for finding nearest station, based on the vehicle type
	 */
	private void processSearchNearestStation() {
		//record starting time of the new request
		this.lstFuelRequestsResponseTime.add(CloudSim.clock());
	...
	private void processReceiveStationData(SimEvent ev) {
		...
		if (stationID == -1) {	//no station found
			Log.printLine(CloudSim.clock() + ": [" + this.getName() + "] haven't received data of nearest fuel station ");
		} else {
	...
	private void checkStationAvailability(int stationID) {
		...
		if (this.getMessagingProtocol() == station.getMessagingProtocol()) {
			//update counters
			this.fuelRequestsTotalMessages += 1;
			schedule(stationID, CloudSim.getMinTimeBetweenEvents(), CloudSimTags.IOV_ELECTRICCHARGINGSTATION_CHECK_AVAILABILITY_EVENT);
		} else {
			//update counters
			this.fuelRequestsTotalMessages += 2;
			schedule(getForwardNodeId(), CloudSim.getMinTimeBetweenEvents(), CloudSimTags.IOV_CLOUD_REQUEST_MEDIATOR);
	...
	private void processReceiveStationAvailability(SimEvent ev) {
		...
		if (isAvailabile) {
			...
			if (this.nearestStationIndex < 5) {
				Log.printLine(CloudSim.clock() + ": [" + this.getName() + "] still searching for the nearest station...");
				...
				schedule(this.getForwardNodeId(), CloudSim.getMinTimeBetweenEvents(), CloudSimTags.IOV_SEARCH_NEAREST_ELECTRICCHARGINGSTATION_EVENT, 
						newData);		
			} else {
				Log.printLine(CloudSim.clock() + ": [" + this.getName() + "] will try later searching for the nearest station...");
	...
	private void updateFuel(int stationID) {
		...
		schedule(connectedVehicleID, timeToStation + CloudSim.getMinTimeBetweenEvents(), CloudSimTags.IOV_VEHICLE_SET_FUEL_FULL_EVENT);
	}

	private void calculateFuelRequestResponseTime() {
		//calculate the service time of the request
		double serviceTime = ((CloudSim.clock() - lstFuelRequestsResponseTime.get(lstFuelRequestsResponseTime.size()-1))
	...
	private void processSearchNearestRestaurant() {
		...
		schedule(this.getForwardNodeId(), CloudSim.getMinTimeBetweenEvents(), CloudSimTags.IOV_SEARCH_NEAREST_RESTAURANT_EVENT, 
				data);	
	...
	private void checkRestaurantAvailability(int restaurantID) {
		Restaurant restaurant = (Restaurant) CloudSim.getEntity(restaurantID);
		...
		if (this.getMessagingProtocol() == restaurant.getMessagingProtocol()) {
			this.restaurantRequestsTotalMessages += 1;
			schedule(restaurantID, CloudSim.getMinTimeBetweenEvents(), CloudSimTags.IOV_RESTAURANT_CHECK_AVAILABILITY_EVENT);
		} else {
	...
	private void processReceiveRestaurantAvailability(SimEvent ev) {
		...
		if (isOpen) {
			//if restaurant is open, proceed with order
			placeOrder(restaurantID);

		} else {
			this.nearestRestaurantIndex +=1;
			if (this.nearestRestaurantIndex < 5) {
				Log.printLine(CloudSim.clock() + ": [" + this.getName() + "] still searching for the nearest restaurant...");
				...
				Log.printLine(CloudSim.clock() + ": [" + this.getName() + "] will try later searching for the nearest restaurant...");
	...
	private void placeOrder(int restaurantID) {
		...
		if (this.getMessagingProtocol() == restaurant.getMessagingProtocol()) {
			this.restaurantRequestsTotalMessages += 1;
			schedule(restaurantID, requestTime, CloudSimTags.IOV_RESTAURANT_ORDER_EVENT);
		} else {
			this.restaurantRequestsTotalMessages += 2;
			schedule(getForwardNodeId(), requestTime, CloudSimTags.IOV_CLOUD_REQUEST_MEDIATOR);
		}
	}
	...
	private void calculateRestaurantRequestResponseTime() {
		//calculate the service time of the request
		double serviceTime = ((CloudSim.clock() - lstRestaurantRequestsResponseTime.get(lstRestaurantRequestsResponseTime.size()-1))
				+ CloudSim.getMinTimeBetweenEvents());
	...
	/**
	 * processSearchNearestParking()
	 * to handle a request for finding nearest parking
	 */
	private void processSearchNearestParking() {		
		...
		schedule(this.getForwardNodeId(), CloudSim.getMinTimeBetweenEvents(), CloudSimTags.IOV_SEARCH_NEAREST_PARKING_EVENT, 
				data);		
	...
	private void processReceiveParkingAvailability(SimEvent ev) {
		...
		if (isAvailabile) {
			...
			if (this.nearestParkingIndex < 5) {
				Log.printLine(CloudSim.clock() + ": [" + this.getName() + "] still searching for the nearest parking...");
				...
				Log.printLine(CloudSim.clock() + ": [" + this.getName() + "] will try later searching for the nearest parking...");
	...
	private void calculateParkingRequestResponseTime() {
		//calculate the service time of the request
		double serviceTime = ((CloudSim.clock() - lstParkingRequestsResponseTime.get(lstParkingRequestsResponseTime.size()-1))
				+ CloudSim.getMinTimeBetweenEvents());
	...
	private void processSearchNearestTrafficControlUnit() {
		...
		schedule(this.getForwardNodeId(), CloudSim.getMinTimeBetweenEvents(), CloudSimTags.IOV_SEARCH_NEAREST_TRAFFICCONTROLUNIT_EVENT, 
				data);		
	...
	private void processReceiveTrafficControlUnitData(SimEvent ev) {
		...
		if (trafficControlUnitID == -1) {	//no traffic control unit found
			Log.printLine(CloudSim.clock() + ": [" + this.getName() + "] haven't received data of nearest traffic control unit ");
		} else {
			Log.printLine(CloudSim.clock() + ": [" + this.getName() + "] received data of the nearest traffic control unit " 
	...
	private void checkTrafficAlert(int trafficControlUnitID) {
		...
		if (this.getMessagingProtocol() == trafficControlUnit.getMessagingProtocol()) {
			this.trafficAlertsTotalMessages +=1;
			schedule(trafficControlUnitID, CloudSim.getMinTimeBetweenEvents(), CloudSimTags.IOV_TRAFFIC_CHECK_ALERT_EVENT);
		} else {
	...
	private void processReceiveTrafficAlertData(SimEvent ev) {
		...
		Log.printLine(CloudSim.clock() + ": [" + this.getName() + "] received the traffic alert set to " 
				+ isTrafficAlert
				+ " from " + CloudSim.getEntityName(trafficControlUnitID)
				);
	...
	private void calculateTrafficAlertResponseTime() {
		//calculate the service time of the request
		double serviceTime = ((CloudSim.clock() - lstTrafficAlertsResponseTime.get(lstTrafficAlertsResponseTime.size()-1))
				+ CloudSim.getMinTimeBetweenEvents());
	 ...
	 */
	/*
	 * private void processCheckTrafficAlert() { Log.printLine(CloudSim.clock() +
	 * ": [" + this.getName() + "] is checking for traffic alert ");
	 * 
	 * Object[] data = new Object[4]; data[0] = this.getCurrentLocation().getX();
	 * data[1] = this.getCurrentLocation().getY(); data[2] =
	 * this.getCurrentLocation().getZ(); data[3] = 0; //to find the first nearest
	 ...
	 * CloudSim.getMinTimeBetweenEvents(),
	 * CloudSimTags.IOV_TRAFFIC_CHECK_ALERT_EVENT, data); }
	 */

	/**
	 * scheduleRestaurantRequest()
	 * schedule restaurant request 
	 */
	private void scheduleRestaurantRequest() {
		...
		if (this.currentExpDay <= ExperimentsConfigurations.EXP_NO_OF_DAYS) {
			...
			if (ExperimentsConfigurations.IOV_EXP_ServiceRequests == "Continuous") {
				startDelay = CloudSim.getMinTimeBetweenEvents() + ExperimentsConfigurations.IOV_EXP_ServiceRequests_INTERVAL;			
	...
	/**
	 * scheduleCheckTrafficAlert()
	 * schedule check traffic alert request   
	 */
	private void scheduleCheckTrafficAlert() {
		...
		if (this.currentExpDay <= ExperimentsConfigurations.EXP_NO_OF_DAYS) {
			...
			//from the datacenter
			//schedule(this.getId(), eventTime, CloudSimTags.IOV_TRAFFIC_CHECK_ALERT_EVENT);
		}
	}
