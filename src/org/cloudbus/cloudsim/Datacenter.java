...
public class Datacenter extends SimEntity {
	...
	public Datacenter(
		...
		if (getCharacteristics().getNumberOfPes() == 0) {
			throw new Exception(super.getName()
					+ " : Error - this entity has no PEs. Therefore, can't process any Service Requests.");
		}

		
		setPower(0.0);
		setOperationalCost(0.0);
		setMigrationCount(0);
	...
	protected void processCloudletStatus(SimEvent ev) {
		...
		try {
			...
			status = getVmAllocationPolicy().getHost(vmId, userId).getVm(vmId,userId).getServiceRequestScheduler()
						.getServiceRequestStatus(cloudletId);
		}
		...
		// if a sender using normal send() methods
			...
			try {
				...
				status = getVmAllocationPolicy().getHost(vmId, userId).getVm(vmId,userId)
							.getServiceRequestScheduler().getServiceRequestStatus(cloudletId);
			} catch (Exception e) {
	...
	/**
	 * Process the event for an User/Broker who wants to create a VM in thisDatacenter. 
	...
	protected void processCloudletMove(int[] receivedData, int type) {
		...
		// get the cloudlet
		ServiceRequest cl = getVmAllocationPolicy().getHost(vmId, userId).getVm(vmId,userId)
				.getServiceRequestScheduler().serviceRequestCancel(cloudletId);
		...
		if (cl == null) {// cloudlet doesn't exist
			...
			if (cl.getServiceRequestStatus() == Cloudlet.SUCCESS) {// if yes, send it back to user
				int[] data = new int[3];
				data[0] = getId();
				data[1] = cloudletId;
				data[2] = 0;
			...
			if (destId == getId()) {
				...
				if (vm == null) {
					...
					vm.getServiceRequestScheduler().serviceRequestSubmit(cl, fileTransferTime);
				}
	...
	protected void processCloudletSubmit(SimEvent ev, boolean ack) {
		...
		try {
			...
			ServiceRequest cl = (ServiceRequest) ev.getData();

			// checks whether this Cloudlet has finished or not
			if (cl.isFinished()) {
				String name = CloudSim.getEntityName(cl.getUserId());
				Log.printLine(getName() + ": Warning - Cloudlet #" + cl.getServiceRequestId() + " owned by " + name
			...
			cl.setResourceParameter(getId(), 
					getCharacteristics().getCostPerCPU(), 
					getCharacteristics().getCostPerMem(), 
					getCharacteristics().getCostPerStorage(), 
			...
			ServiceRequestScheduler scheduler = vm.getServiceRequestScheduler();
			double estimatedFinishTime = scheduler.serviceRequestSubmit(cl, fileTransferTime);
	...
	protected void processCloudletResume(int cloudletId, int userId, int vmId, boolean ack) {
		double eventTime = getVmAllocationPolicy().getHost(vmId, userId).getVm(vmId,userId)
				.getServiceRequestScheduler().serviceRequestResume(cloudletId);
	...
	protected void processCloudletPause(int cloudletId, int userId, int vmId, boolean ack) {
		boolean status = getVmAllocationPolicy().getHost(vmId, userId).getVm(vmId,userId)
				.getServiceRequestScheduler().serviceRequestPause(cloudletId);
	...
	protected void checkCloudletCompletion() {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			for (Vm vm : host.getVmList()) {
				while (vm.getServiceRequestScheduler().isFinishedServiceRequests()) {
					ServiceRequest cl = vm.getServiceRequestScheduler().getNextFinishedServiceRequest();
					if (cl != null) {
						sendNow(cl.getUserId(), CloudSimTags.CLOUDLET_RETURN, cl);
					}
				}
...
/*    *//**
	...
	public void setOperationalCost() { 	
		this.operationalCost = calculateOperationalCost();
	}
