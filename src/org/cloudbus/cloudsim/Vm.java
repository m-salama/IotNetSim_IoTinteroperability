...
public class Vm {
	...
	private ServiceRequestScheduler cloudletScheduler;
	...
	private List<ServiceRequest> assignedCloudlets = new ArrayList<ServiceRequest>();
	...
	public Vm(
		...
		setSize(size);
		setVmm(vmm);
		setServiceRequestScheduler(cloudletScheduler);
	...
	public double updateVmProcessing(double currentTime, List<Double> mipsShare) {
		if (mipsShare != null) {
			return getServiceRequestScheduler().updateVmProcessing(currentTime, mipsShare);
		}
		return 0.0;
	}
	...
	public List<Double> getCurrentRequestedMips() {
		List<Double> currentRequestedMips = getServiceRequestScheduler().getCurrentRequestedMips();
	...
	public long getCurrentRequestedBw() {
		...
		return (long) (getServiceRequestScheduler().getCurrentRequestedUtilizationOfBw() * getBw());
	}
	...
	public int getCurrentRequestedRam() {
		...
		return (int) (getServiceRequestScheduler().getCurrentRequestedUtilizationOfRam() * getRam());
	}
	...
	public double getTotalUtilizationOfCpu(double time) {
		return getServiceRequestScheduler().getTotalUtilizationOfCpu(time);
	}
	 ...
	 * Sets the vm type.
	 * 
	 * @param vm type index in the Constants 
	 */
	...
	/**
	 * Changes the vm type and all associated properties.
	 * 
	 * @param vm type index in the Constants 
	 */
	public void changeType(int newType) {
		this.setType(newType);
		this.setMips(Constants.VM_MIPS[newType]);
		this.setNumberOfPes(Constants.VM_PES[newType]);
		this.setRam(Constants.VM_RAM[newType]); 
		this.setBw(Constants.VM_BW[newType]); 
		this.setSize(Constants.VM_SIZE[newType]);
		this.setVmm(Constants.VM_MONITOR[newType]); 
	}
	 ...
	 */
	public ServiceRequestScheduler getServiceRequestScheduler() {
	 ...
	 */
	protected void setServiceRequestScheduler(ServiceRequestScheduler cloudletScheduler) {
	...
	public List<ServiceRequest> getAssignedServiceRequestList() {
		return assignedCloudlets;
	}
	
	public List<ServiceRequest> getWaitingServiceRequestList() {
		List<ServiceRequest> waitingServiceRequests = new ArrayList<ServiceRequest>();

		for (ServiceRequest cloudlet : getAssignedServiceRequestList()) 
			if (cloudlet.getServiceRequestStatus() == ServiceRequest.QUEUED)  //status = QUEUED
				waitingServiceRequests.add(cloudlet);

		return waitingServiceRequests;
	}
	
	public List<ServiceRequest> getInExecServiceRequestList() {
		List<ServiceRequest> inExecServiceRequests = new ArrayList<ServiceRequest>();

		for (ServiceRequest cloudlet : getAssignedServiceRequestList())
			if (cloudlet.getServiceRequestStatus() == ServiceRequest.INEXEC) //status = INEXEC
				inExecServiceRequests.add(cloudlet);

		return inExecServiceRequests;
	}

	public List<ServiceRequest> getFinishedServiceRequestList() {
		List<ServiceRequest> finishedServiceRequests = new ArrayList<ServiceRequest>();

		for (ServiceRequest cloudlet : getAssignedServiceRequestList()) 
			if (cloudlet.getServiceRequestStatus() == ServiceRequest.SUCCESS) //status = SUCCESS
				finishedServiceRequests.add(cloudlet);

		return finishedServiceRequests;
	}
