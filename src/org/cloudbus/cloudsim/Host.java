...
public class Host {
	...
	/** The machine type (for physical configuration) of the host. */
	private int type;
	...
	/**
	 * Turns on a VM running in the host.
	 * 
	 * @param vm the VM
	 * @pre $none
	 * @post $none
	 */
	public void vmEnable(Vm vm) {
		((AdvVm) VmList.getById(getVmList(), vm.getId())).setOn(true);
	}

	/**
	 * Turns off a VM running in the host.
	 * 
	 * @param vm the VM
	 * @pre $none
	 * @post $none
	 */
	public void vmDisable(Vm vm) {
		//vmDeallocate(vm);
		((AdvVm) VmList.getById(getVmList(), vm.getId())).setOn(false);
	}
